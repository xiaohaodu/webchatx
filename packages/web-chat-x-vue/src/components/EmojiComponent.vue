...
const refEmojiPicker = ref() as Ref<Picker>;
const picker = new Picker({
  // i18n: zh_CN,
  // locale: "zh_CN",
});
const emit = defineEmits<{
  "emoji-click": [value: string];
}>();
onMounted(() => {
  refEmojiPicker.value.appendChild(picker);
  picker.addEventListener("emoji-click", (emoji) => {
    emit("emoji-click", emoji.detail.unicode!);
  });
});
</script>
