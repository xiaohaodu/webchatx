<template>
  <el-container class="h-full flex">
    <el-header class="bg-gray-100 flex items-center justify-between">
      <el-tooltip :content="`${channel.name}@${channel.id}`" placement="top">
        <h2 class="text-lg font-semibold break-all line-clamp-1 w-full" >{{ `${channel.name}@${channel.id}` }}</h2>
     </el-tooltip>
      <span class="text-gray-400 text-sm"
        >{{ channel.friendIds.length }} people chatting</span
      >
    </el-header>
    <el-main ref="refMain" class="flex-grow-1 flex-shrink-1 overflow-y-auto">
      <div v-for="(message, index) in messages?.messages"
        ...
        <div v-else class="flex flex-row items-start">
          <img
            :src="libp2pManager.getSubscriber(message.postUserId)!.avatar"
            alt="User Avatar"
            class="w-8 h-8 rounded-full"
          />
          <p
    ...
    <el-footer class="flex flex-col flex-shrink-0" height="auto">
      <el-row class="flex flex-row justify-between">
        ...
        <template #trigger>
          ...
          <!-- <span class="m-1 icon-[f7--folder]" @click="sendFile"></span> -->
          <!-- <span class="m-1 icon-[solar--scissors-outline]"></span> -->
          <!-- <span class="m-1 icon-[mingcute--message-3-line]"></span> -->
        </div>
        <div class="cursor-pointer flex flex-row">
          <!-- <span class="m-1 icon-[solar--phone-outline]" @click="audioCall"></span> -->
          <!-- <span class="m-1 icon-[ri--vidicon-line]" @click="videoCall"></span> -->
        </div>
      </el-row>
      <el-row>
        <el-col :span="22">
...
const channelId=computed(()=>{
  return  route.params.channel_id as string
})
const currentUser= libp2pManager.getChatUser()
const [messages,channel]=await Promise.all([
ref(await libp2pManager.getMessageAggregation("",currentUser?.value.id,channelId.value))
,ref(( libp2pManager.getChannel(channelId.value))!)
])
watchEffect(async ()=>{
  messages.value=await libp2pManager.getMessageAggregation("",currentUser?.value.id,channelId.value)
  channel.value=(libp2pManager.getChannel(channelId.value))!
})
onMounted(()=>{
  nextTick(()=>{
...
async function sendMessage() {
  if(messageText.value){
    messages.value=await libp2pManager.publishSubscribeMessage(channel.value,messageText.value);
    messageText.value = "";
  }else{
    ElMessage({
      type:"info",
...
async function isOnlineProxy(handle:Function|(()=>{})){
  if(channel.value.friendIds.length){
    handle()
  }else{
    ElMessage({
      type:"info",
...
// peerManager.remotePeerId=channelId
// peerManager.remoteUser=ref(channel)
...
//   libp2pManager.sendFile(channel.value,uploadFile.raw!)
...
const cyclicQuery=()=>{
  ...
  const cyclicFunction=async ()=>{
    const messageTemplate=await libp2pManager.getMessageAggregation("",currentUser.value.id,channelId.value,channelId.value)
